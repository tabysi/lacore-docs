...
export const SECTIONS = [
...

  {
    ...
    fields: [
      ...
      { id: 'sync_time', file: 'configs/config.lua', path: 'SyncGameTime',
        type: 'bool', default: false, label: 'LACORE owns the world clock',
...

  {
    ...
    fields: [
      ...
      { id: 'duty_ace', file: 'configs/cfg-server-sv.lua', path: 'DutyAccess.enabled',
        type: 'bool', default: false, label: 'Gate agencies by role / ACE',
        help: 'ON = which agency a player may go on duty for is checked against a Discord role or an ACE permission, which also decides which MDT portal they get. The per-agency list itself lives in the config file — this is the master switch.' },
      { id: 'duty_staff', file: 'configs/cfg-server-sv.lua', path: 'DutyAccess.bypassStaff',
        type: 'bool', default: true, label: 'Staff bypass the agency gate',
        help: 'Let Staff/Dev take any agency without its role or ACE. Turn OFF for a strict server.' },
      { id: 'duty_fallback', file: 'configs/cfg-server-sv.lua', path: 'DutyAccess.fallback',
        type: 'select', default: 'allow', options: [['allow', 'Allow (only listed agencies are gated)'], ['deny', 'Deny (strict allowlist)']],
        label: 'Agencies with no entry',
        help: 'What happens to an agency you did not list: open to everyone, or closed until you give it a rule.' },
      { id: 'vehspawn_perm', file: 'configs/cfg-server-sv.lua', path: 'VehicleSpawner.permission',
        type: 'text', default: '', label: '/vehicle permission',
        help: 'Empty = everyone may open the vehicle spawner (the historical behaviour). Set an ACE like "vehiclespawner" to restrict it, then grant it in server.cfg: add_ace group.admin lacore.vehiclespawner allow.' },
    ],
  },
...

  {
    ...
    fields: [
      ...
      { id: 'hud_crosshair', file: 'configs/cfg-hud-sh.lua', path: 'HudCfg.crosshair', type: 'bool', default: false, label: 'Weapon crosshair', help: "ON = GTA's reticle stays visible for every weapon. OFF (default) = LACORE hides it; sniper scopes always keep theirs." },
    ],
  },
...

  {
    ...
    fields: [
      ...
      { id: 'ph_eyefind', file: 'configs/cfg-phone-sh.lua', path: 'PhoneCfg.eyefindUrl', type: 'text', default: 'https://lacore.tebex.io/', label: 'Eyefind browser home', help: 'The in-phone browser home page — point it at YOUR community, never a foreign site.' },
...

  {
    ...
    fields: [
      { id: 'ac_enabled', file: 'configs/cfg-anticheat-sh.lua', path: 'Anticheat.Enabled', type: 'bool', default: true, label: 'Enable anti-cheat', help: 'Master switch. Off by default; turn on once you’ve reviewed the actions below.' },
...

  {
    ...
    fields: [
      { id: 'rp_enabled', file: 'configs/cfg-replay-sh.lua', path: 'Replay.enabled',
        type: 'bool', default: false, label: 'Replay enabled', help: 'Ships off. Also gated by the replay feature toggle — either one off means off.' },
...
// ── Starting points ──────────────────────────────────────────────────────────
// A template is not a different product — it is the answer to "what would you
// change first on a server like mine". Each one sets a handful of fields and
// leaves everything else on its default, so the generated config stays small and
// you can still walk every step afterwards.
//
// `values` keys are field ids from SECTIONS above. Anything not listed keeps its
// default, and a template never invents a value the config would not accept.
export const TEMPLATES = [
  {
    id: 'leo',
    name: 'Law-enforcement roleplay',
    icon: '🚓',
    blurb:
      'The CAD suite is the point: every terminal on, civilian extras off, and the investigative ' +
      'settings turned up — prints must be taken before they identify anyone.',
    values: {
      f_civilian: false,
      f_extras: false,
      f_webdispatch: false,
  ...
  {
    id: 'framework',
    name: 'ESX / QBCore server',
    icon: '🔗',
    blurb:
      'Your framework owns identity, jobs and plates. LACORE reads them instead of competing: its ' +
      'own whitelist and Discord duty roles step aside, and plate re-formatting stays off so your ' +
      'garage keeps working.',
    values: {
      ac_whitelist: false,
      ac_discord: false,
      veh_plate: 'false',
  ...
  {
    id: 'ace',
    name: 'vMenu / ACE-driven server',
    icon: '🔑',
    blurb:
      'Permissions already come from Discord roles mapped to ACE groups. LACORE stops asking for ' +
      'Discord itself and takes its answers from ACE — including which agency a unit may take and ' +
      'who may open the vehicle spawner.',
    values: {
      ac_discord: false,
      ac_whitelist: false,
      duty_ace: true,
      duty_fallback: 'deny',
      vehspawn_perm: 'vehiclespawner',
    },
  },
  {
    id: 'plugandplay',
    name: 'Small community, no setup',
    icon: '🌱',
    blurb:
      'Nothing to configure before people can play: no whitelist, no Discord requirement, no ' +
      'database. Everything stores locally as JSON and every module stays on.',
    values: {
      ac_whitelist: false,
      ac_discord: false,
      use_db: false,
  ...
  {
    id: 'performance',
    name: 'Large server, performance first',
    icon: '⚡',
    blurb:
      'Turns off the features that cost the most per player — a browser per passenger seat, clip ' +
      'recording, speech recognition, turf war — and keeps the audit log shorter.',
    values: {
      vs_passengers: false,
      vs_tick: 2000,
      rp_enabled: false,
