...
This is what makes plate runs interesting: **if a plate isn't on file and someone is driving it, they get 15 seconds to register it — or the *plate* is flagged stolen.** (The vehicle is never deleted — only the plate gets a stolen flag. See [Registering a vehicle](/user-guide/civilian-vehicles).)
...
- Every run is announced in the in-game chat to on-duty units (`MDT` prefix, blue). Server owners
  can turn this off with `MdtConfig.announceRuns = false` in `configs/cfg-mdt-sh.lua` (LASD/CAD
  runs included).
- Runs are logged in the admin audit (admin-only access via `/calllog`).
- Developers who want to look up records from their own scripts use the read-only
  [`QueryPerson` / `QueryPlate` exports](/developer-api/server) — results are sanitised, private
  records never leave the server.
