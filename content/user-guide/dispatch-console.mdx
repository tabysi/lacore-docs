...
> 📑 **Related:** [Using the MDT](/user-guide/mdt) · [Working with Incidents](/user-guide/mdt-incidents) · [Status labels](/user-guide/statuses)
...
| Element | Purpose |
|---|---|
| **DISPATCH CONSOLE** | Title — the region is detected automatically |
...
| 🚓 `<callsign>` | A live on-duty unit (coloured dot by status) |
...
Each row shows the **callsign**, current **status** (coloured badge), and **incident number** if attached.
...
Each map area belongs to a **region** (e.g. "Los Angeles", "Compton", "Thousand Oaks"). By default the framework auto-detects the region from the GTA zone code; the **Zone Editor** lets you **override** that mapping per zone.
...
| `ShowIncidentBlips` | Whether on-duty units see GTA map blips for active incidents (default: `false`) |
...
- [Status labels](/user-guide/statuses) — what the colour-coded badges across the CAD mean
