...
3. **If it's *not* registered** and you're the driver, you get a prompt:
   ...
   - **Time out / close it** → the **plate** is **flagged stolen** in the CAD.

> **LACORE never deletes the vehicle here.** The `/vreg`-or-stolen flow only writes a
> *stolen flag* on the **plate** so officers see it when they run it — the car itself
> is untouched. If a vehicle disappears the moment it spawns, that is a **different
> resource** (a vehicle whitelist, an anticheat, or a garage/framework script), not
> this system. You can turn the whole flow off with `Vehicles.vreg.enabled = false`
> in `configs/cfg-vehicles-sh.lua` (or from the dashboard), and change the window with
> `Vehicles.vreg.seconds` — but that will not stop another resource from deleting cars.
