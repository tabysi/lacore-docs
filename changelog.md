...
- **Config data-list editor** (`/dashboard/config/lists`): edit LACORE's config *tables* — starting with
  ...
  in the resource to expose more tables (vehicles, blips, …). Now also ships two flat vehicle lists — **blacklisted vehicles** and **member-only vehicles** — edited as a simple one-per-line textarea (spawn names or class numbers, deduped).
