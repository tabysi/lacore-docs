...
#### CAD & MDT
- **A Supervisor Panel — a rank-gated oversight tablet.** `/supervisor` opens a tablet for shift
  supervisors and watch commanders with five views: a **live unit roster** (status, rank, incident and
  location, sortable and searchable), a **live map** of every unit and active incident, **dispatch
  controls** (assign a unit to an incident, set its status), a **division broadcast** (message to all
  units + the attention tone), and **BOLO / warrant oversight** (active BOLOs with one-click cancel, and
  every character currently carrying a warrant). Who counts as a supervisor is **server-decided**: the
  unit's **job grade** on a framework server (per-department minimum), or a **config allowlist** of
  license identifiers / callsigns for standalone servers. Gating is enforced server-side for the panel
  *and* its actions — the client only asks — and it reuses the existing dispatch backend, so a supervisor
  no longer has to be registered as a dispatcher to direct their shift.

  > ⚠️ **New config file (`configs/cfg-supervisor-sh.lua`):** who may open the panel — `minGrade` /
